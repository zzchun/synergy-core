...
XWindowsClipboard::XWindowsClipboard (Display* display, Window window,
    ...
    m_atomMotifClipHeader =
        XInternAtom (m_display, "_MOTIF_CLIP_HEADER", False);
...
void
XWindowsClipboard::addRequest (Window owner, Window requestor, Atom target,
...
bool
XWindowsClipboard::addSimpleRequest (Window requestor, Atom target, ::Time time,
                                     Atom property) {
...
bool
XWindowsClipboard::processRequest (Window requestor, ::Time /*time*/,
                                   Atom property) {
...
XWindowsClipboard::add (EFormat format, const String& data) {
    ...
    LOG ((CLOG_DEBUG "add %d bytes to clipboard %d format: %d",
          data.size (),
          m_id,
          format));
...
XWindowsClipboard::getConverter (Atom target, bool onlyIfNotAdded) const {
    ...
    if (onlyIfNotAdded) {
        if (m_added[converter->getFormat ()]) {
            LOG ((CLOG_DEBUG1 "  skipping handled format %d",
                  converter->getFormat ()));
            return nullptr;
        }
    }
...
XWindowsClipboard::icccmFillCache () {
    ...
    const auto* targets =
        reinterpret_cast<const Atom*> (data.data ()); // TODO(andrew): Safe?
    const UInt32 numTargets = data.size () / sizeof (Atom);
    LOG ((
        CLOG_DEBUG "  available targets: %s",
    ...
    for (ConverterList::const_iterator index = m_converters.begin ();
        ...
        */
        ...
        m_added[format]            = true;
        LOG ((CLOG_DEBUG "added format %d for target %s (%u %s)",
              format,
              XWindowsUtil::atomToString (m_display, target).c_str (),
              targetData.size (),
...
bool
XWindowsClipboard::icccmGetSelection (Atom target, Atom* actualTarget,
                                      String* data) const {
...
XWindowsClipboard::motifFillCache () {
    ...
    data          = "";
    if (!XWindowsUtil::getWindowProperty (
            m_display, root, atomItem, &data, &target, &format, False)) {
        return;
    }
    ...
    const auto* formats     = reinterpret_cast<const SInt32*> (
    ...
    for (SInt32 i = 0; i < numFormats; ++i) {
        ...
        String data;
        if (!XWindowsUtil::getWindowProperty (
                m_display, root, atomFormat, &data, &target, &format, False)) {
            continue;
        }
    ...
    for (ConverterList::const_iterator index = m_converters.begin ();
        ...
        MotifClipFormat motifFormat{};
        std::memcpy (
            &motifFormat, index2->second.data (), sizeof (motifFormat));
        ...
        m_data[format]             = converter->toIClipboard (targetData);
        m_added[format]            = true;
        LOG ((CLOG_DEBUG "added format %d for target %s",
              format,
              XWindowsUtil::atomToString (m_display, target).c_str ()));
    }
}
...
XWindowsClipboard::motifGetSelection (const MotifClipFormat* format,
    ...
    Window root = RootWindow (m_display, DefaultScreen (m_display));
    return XWindowsUtil::getWindowProperty (
        m_display, root, target, data, actualTarget, nullptr, False);
}
...
bool
XWindowsClipboard::insertMultipleReply (Window requestor, ::Time time,
                                        Atom property) {
    ...
    String data;
    if (!XWindowsUtil::getWindowProperty (
            m_display, requestor, property, &data, &target, &format, False)) {
    ...
    XWindowsUtil::convertAtomProperty (data);
    const auto* targets     = reinterpret_cast<const Atom*> (data.data ());
    const UInt32 numTargets = data.size () / sizeof (Atom);
    ...
    insertReply (new Reply (
        requestor, m_atomMultiple, time, property, String (), None, 32));
                    ...
                    for (String::size_type j = 0; j < data.size (); ++j) {
                        if (data[j] < 32 || data[j] > 126) {
                            ...
                            for (j = 0; j < data.size (); ++j) {
                                auto v = static_cast<unsigned char> (data[j]);
                                tmp += hex[v >> 16];
                                tmp += hex[v & 15];
                                tmp += ' ';
                            }
...
XWindowsClipboard::clearReplies () {
    for (auto& m_replie : m_replies) {
        clearReplies (m_replie.second);
    }
    m_replies.clear ();
    m_eventMasks.clear ();
}
...
void
XWindowsClipboard::sendNotify (Window requestor, Atom selection, Atom target,
...
XWindowsClipboard::CICCCMGetClipboard::readClipboard (Display* display,
    ...
    *m_data         = "";
    ...
    XSelectInput (
        display, m_requestor, attr.your_event_mask | PropertyChangeMask);

    // request data conversion
    XConvertSelection (
        display, selection, target, m_property, m_requestor, m_time);
    ...
    LOG ((CLOG_DEBUG1 "request %s after %fs",
          m_failed ? "failed" : "succeeded",
          timeout.getTime ()));
    return !m_failed;
}

bool
XWindowsClipboard::CICCCMGetClipboard::processEvent (Display* display,
                                                     XEvent* xevent) {
    // process event
    switch (xevent->type) {
        case DestroyNotify:
        ...
        case SelectionNotify:
            if (xevent->xselection.requestor == m_requestor) {
                ...
                if (xevent->xselection.property == m_property) {
                    m_reading = true;
                    break;
                }
            }
    ...
    if (!XWindowsUtil::getWindowProperty (
            display, m_requestor, m_property, m_data, &target, nullptr, True)) {
    ...
    // handle incremental chunks
        ...
        if (m_data->size () == oldSize) {
            LOG ((CLOG_DEBUG1 "  INCR final chunk: %d bytes total",
                  m_data->size ()));
            m_done = true;
        }
    }
