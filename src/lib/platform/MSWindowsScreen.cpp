...
bool
MSWindowsScreen::onPreDispatchSecondary (HWND, UINT message, WPARAM wParam,
                                         LPARAM lParam) {
    switch (message) {
        case SYNERGY_MSG_KEY:
        case SYNERGY_MSG_MOUSE_BUTTON:
...
MSWindowsScreen::warpCursorNoFlush (SInt32 x, SInt32 y) {
    ...
    if (!isPrimary ()) {
        if ((cursorPos.x != x) && (cursorPos.y != y)) {
            ...
            // could be an MS security feature). instead we can use
            // fakeMouseMove, which calls mouse_event. IMPORTANT: as of
            // implementing this function, it has an annoying side effect;
            // instead of the mouse returning to the correct exit point, it
            // returns to the center of the screen. this could have something to
            // do with the center screen warping technique used (see comments
            // for onMouseMove definition).
            fakeMouseMove (x, y);
        }
    }
