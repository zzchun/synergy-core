...
XWindowsKeyState::updateKeysymMap (synergy::KeyMap& keyMap) {
    ...
    KeySym* allKeysyms = XGetKeyboardMapping (
        m_display, minKeycode, numKeycodes, &keysymsPerKeycode);
    ...
    for (int i = 0; i < numKeycodes; ++i) {
        ...
        for (int j = 0; j < maxKeysyms; ++j) {
            ...
            if ((j & 1) != 0) {
                ...
                if (IsKeypadKey (keysyms[j]) ||
                    IsPrivateKeypadKey (keysyms[j])) {
                    item.m_required &= ~KeyModifierShift;
            ...
            for (int j = -1; j < type->map_count; ++j) {
                ...
                if (isModifier && modifierBit != kKeyModifierBitNone) {
                    ...
                    for (SInt32 j = 0; j < 8; ++j) {
                        ...
                        m_modifierToX.insert (
                            std::make_pair (1u << modifierBit, 1u << j));
                    }
                }
                ...
                if (group == 0) {
                    m_keyCodeFromKey.insert (
                        std::make_pair (item.m_id, keycode));
                }
            }
        }
    }
