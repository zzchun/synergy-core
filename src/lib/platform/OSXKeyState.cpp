...
OSXKeyState::OSXKeyState (IEventQueue* events) : KeyState (events) {
    init ();
}
...
KeyButton
OSXKeyState::mapKeyFromEvent (KeyIDs& ids, KeyModifierMask* maskOut,
    ...
    } else if (CGEventGetIntegerValueField (event,
...
OSXKeyState::pollActiveGroup () const {
    ...
    LOG (
        (CLOG_DEBUG "can't get the active group, use the first group instead"));
...
OSXKeyState::fakeKey (const Keystroke& keystroke) {
    switch (keystroke.m_type) {
        case Keystroke::kButton: {
            ...
            LOG ((CLOG_DEBUG1 "  button=0x%04x virtualKey=0x%04x keyDown=%s",
                  button,
                  virtualKey,
                  keyDown ? "down" : "up"));
...
void
OSXKeyState::getKeyMapForSpecialKeys (synergy::KeyMap& keyMap,
                                      SInt32 group) const {
...
bool
OSXKeyState::getKeyMap (synergy::KeyMap& keyMap, SInt32 group,
    ...
    for (UInt32 i = 0; i < r.getNumButtons (); ++i) {
        ...
        for (UInt32 j = 0; j < r.getNumTables (); ++j) {
            ...
            buttonKeys[j].second =
                synergy::KeyMap::isDeadKey (buttonKeys[j].first);
        }
        ...
        for (UInt32 j = 0; j < r.getNumTables (); ++j) {
            ...
            UInt32 sensitive = 0;
            for (UInt32 k = 0; (1u << k) < r.getNumModifierCombinations ();
                 ++k) {
                UInt32 bit = (1u << k);
...
void
OSXKeyState::handleModifierKeys (void* target, KeyModifierMask oldMask,
...
void
OSXKeyState::handleModifierKey (void* target, UInt32 virtualKey, KeyID id,
...
void
OSXKeyState::adjustAltGrModifier (const KeyIDs& ids, KeyModifierMask* mask,
