...
XWindowsUtil::atomToString (Display* display, Atom atom) {
    ...
    if (error) {
        return synergy::string::sprintf ("<UNKNOWN> (%d)",
                                         static_cast<int> (atom));
    }

    String msg =
        synergy::string::sprintf ("%s (%d)", name, static_cast<int> (atom));
    XFree (name);
    return msg;
}
...
XWindowsUtil::atomsToString (Display* display, const Atom* atom, UInt32 num) {
    ...
    XWindowsUtil::ErrorLock lock (display, &error);
    XGetAtomNames (
        display, const_cast<Atom*> (atom), static_cast<int> (num), names);
    String msg;
    if (error) {
        for (UInt32 i = 0; i < num; ++i) {
            msg += synergy::string::sprintf ("<UNKNOWN> (%d), ",
                                             static_cast<int> (atom[i]));
        }
    } else {
        for (UInt32 i = 0; i < num; ++i) {
            msg += synergy::string::sprintf (
                "%s (%d), ", names[i], static_cast<int> (atom[i]));
            XFree (names[i]);
        }
    }
    delete[] names;
...
Bool
XWindowsUtil::propertyNotifyPredicate (Display* /*unused*/, XEvent* xevent,
                                       XPointer arg) {
...
void
XWindowsUtil::ErrorLock::ignoreHandler (Display* /*unused*/, XErrorEvent* e,
                                        void* /*unused*/) {
    LOG ((CLOG_DEBUG1 "ignoring X error: %d", e->error_code));
}

void
XWindowsUtil::ErrorLock::saveHandler (Display* /*unused*/, XErrorEvent* e,
                                      void* flag) {
