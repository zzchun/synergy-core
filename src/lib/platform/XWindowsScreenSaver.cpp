...
XWindowsScreenSaver::XWindowsScreenSaver (Display* display, Window window,
                                          void* eventTarget,
                                          IEventQueue* events)
    : m_display (display),
    ...
    m_atomScreenSaverVersion =
        XInternAtom (m_display, "_SCREENSAVER_VERSION", False);
    ...
    if (DPMSQueryExtension (m_display, &eventBase, &errorBase) != 0) {
        if (DPMSCapable (m_display) != 0) {
            // we have DPMS
            m_dpms = true;
        }
    }
    ...
    bool error = false;
    {
        ...
        XSelectInput (
            m_display, root, m_rootEventMask | SubstructureNotifyMask);
    }
    if (error) {
...
XWindowsScreenSaver::~XWindowsScreenSaver () {
    ...
    if (m_display != nullptr) {
        ...
        XSelectInput (
            m_display, DefaultRootWindow (m_display), m_rootEventMask);
    }
}
...
XWindowsScreenSaver::disable () {
    ...
    XGetScreenSaver (m_display,
                     ...
                     &m_allowExposures);
    XSetScreenSaver (
        m_display, 0, m_interval, m_preferBlanking, m_allowExposures);
...
XWindowsScreenSaver::findXScreenSaver () {
    ...
    if (m_xscreensaver == None) {
        ...
        Window rw, pw, *cw;
        unsigned int nc;
        if (XQueryTree (m_display, root, &rw, &pw, &cw, &nc) != 0) {
...
XWindowsScreenSaver::isXScreenSaver (Window w) const {
    ...
    Atom type;
    return (XWindowsUtil::getWindowProperty (m_display,
                                             w,
                                             m_atomScreenSaverVersion,
                                             nullptr,
                                             &type,
...
XWindowsScreenSaver::sendXScreenSaverCommand (Atom cmd, long arg1, long arg2) {
    ...
    LOG ((CLOG_DEBUG "send xscreensaver command: %d %d %d",
          (long) cmd,
          arg1,
          arg2));
    bool error = false;
...
XWindowsScreenSaver::clearWatchForXScreenSaver () {
    ...
    XWindowsUtil::ErrorLock lock (m_display);
    for (auto& m_watchWindow : m_watchWindows) {
        XSelectInput (m_display, m_watchWindow.first, m_watchWindow.second);
    }
...
XWindowsScreenSaver::addWatchXScreenSaver (Window window) {
    ...
    if (!error && attr.override_redirect == True) {
        error = false;
        {
            XWindowsUtil::ErrorLock lock (m_display, &error);
            XSelectInput (
                m_display, window, attr.your_event_mask | PropertyChangeMask);
        }
        if (!error) {
            // if successful then add the window to our list
            m_watchWindows.insert (
                std::make_pair (window, attr.your_event_mask));
        }
    }
}
...
XWindowsScreenSaver::updateDisableTimer () {
    ...
    } else if ((!m_disabled || m_suppressDisable) &&
               m_disableTimer != nullptr) {
        m_events->deleteTimer (m_disableTimer);
        m_disableTimer = nullptr;
    }
...
void
XWindowsScreenSaver::handleDisableTimer (const Event& /*unused*/,
                                         void* /*unused*/) {
