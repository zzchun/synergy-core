...
ServerProxy::handleData (const Event& /*unused*/, void* /*unused*/) {
    ...
    while (n != 0) {
        ...
        // parse message
        LOG ((CLOG_DEBUG2 "msg from server: %c%c%c%c",
              code[0],
              code[1],
              code[2],
              code[3]));
        switch ((this->*m_parser) (code)) {
            ...
            case kUnknown:
                LOG ((CLOG_ERR "invalid message from server: %c%c%c%c",
                      code[0],
                      code[1],
                      code[2],
                      code[3]));
...
ServerProxy::setClipboard () {
    ...
    if (r == kStart) {
        ...
        LOG ((CLOG_DEBUG "received clipboard %d size=%d",
              id,
              dataCached.size ()));
...
ServerProxy::keyRepeat () {
    ...
    ProtocolUtil::readf (
        m_stream, kMsgDKeyRepeat + 4, &id, &mask, &count, &button);
    LOG ((CLOG_DEBUG1
    ...
    if (id2 != static_cast<KeyID> (id) ||
        mask2 != static_cast<KeyModifierMask> (mask)) {
        LOG ((CLOG_DEBUG1 "key repeat translated to id=0x%08x, mask=0x%04x",
              id2,
              mask2));
    }
...
ServerProxy::keyUp () {
    ...
    LOG ((CLOG_DEBUG1 "recv key up id=0x%08x, mask=0x%04x, button=0x%04x",
          id,
          mask,
          button));
    ...
    if (id2 != static_cast<KeyID> (id) ||
        mask2 != static_cast<KeyModifierMask> (mask)) {
        LOG ((CLOG_DEBUG1 "key up translated to id=0x%08x, mask=0x%04x",
              id2,
              mask2));
    }
...
ServerProxy::setOptions () {
    ...
    for (UInt32 i = 0, n = static_cast<UInt32> (options.size ()); i < n;
        ...
        if (id != kKeyModifierIDNull) {
            ...
            LOG ((CLOG_DEBUG1 "modifier %d mapped to %d",
                  id,
                  m_modifierTranslationTable[id]));
        }
    }
...
ServerProxy::fileChunkReceived () {
    int result = FileChunk::assemble (m_stream,
    ...
    if (result == kFinish) {
        ...
        if (!m_client->getDragFileList ().empty ()) {
            String filename =
                m_client->getDragFileList ().at (0).getFilename ();
...
void
ServerProxy::handleClipboardSendingEvent (const Event& event,
                                          void* /*unused*/) {
