...
static const KeyButton kButtonMask =
    static_cast<KeyButton> (IKeyState::kNumButtons - 1);
...
KeyState::updateModifierKeyState (KeyButton button,
    ...
    for (const auto& oldModifier : oldModifiers) {
        oldKeys.insert (
            std::make_pair (oldModifier.second.m_button, &oldModifier.second));
    }
    for (const auto& newModifier : newModifiers) {
        newKeys.insert (
            std::make_pair (newModifier.second.m_button, &newModifier.second));
    }
