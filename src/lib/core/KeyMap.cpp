...
KeyMap::findCompatibleKey (KeyID id, SInt32 group, KeyModifierMask required,
    ...
    for (const auto& entrie : entries) {
        if ((entrie.back ().m_sensitive & sensitive) == 0 ||
            (entrie.back ().m_required & sensitive) == (required & sensitive)) {
            return &entrie;
        }
    }
            ...
            for (const auto& entrie : entries) {
                ...
                for (SInt32 b = 0; b < kKeyModifierNumBits; ++b) {
                    ...
                    if (((1u << b) & item.m_generates) != 0) {
                        SInt32 mIndex =
                            static_cast<SInt32> (g) * kKeyModifierNumBits + b;
...
KeyMap::keysForModifierState (KeyButton button, SInt32 group,
    ...
    for (SInt32 bit = kKeyModifierNumBits; bit-- > 0;) {
        ...
        if (sensitive != 0) {
            ...
            if (active && !keysForModifierState (button,
                                                 group,
                                                 activeModifiers,
                                                 currentState,
                                                 keyItem->m_required,
...
KeyMap::initKeyNameMaps () {
    ...
    if (s_nameToKeyMap == nullptr) {
        ...
        for (const KeyNameMapEntry* i = kKeyNameMap; i->m_name != nullptr;
             ++i) {
