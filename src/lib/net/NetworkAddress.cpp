...
NetworkAddress::NetworkAddress () : m_address (nullptr), m_port (0) {
    // note -- make no calls to Network socket interface here;
    // we're often called prior to Network::init().
}

NetworkAddress::NetworkAddress (int port) : m_address (nullptr), m_port (port) {
    checkPort ();
...
NetworkAddress::NetworkAddress (const NetworkAddress& addr)
    : m_address (addr.m_address != nullptr ? ARCH->copyAddr (addr.m_address)
                                           : nullptr),
...
NetworkAddress::NetworkAddress (String hostname, int port)
    : m_address (nullptr), m_hostname (std::move (hostname)), m_port (port) {
        ...
        if ((!doubleColon || dotNotation) || !colonNotation) {
            ...
            if (end == chostname + i + 1 || *end != '\0') {
                throw XSocketAddress (
                    XSocketAddress::kBadPort, m_hostname, m_port);
            }
