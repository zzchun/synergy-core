...
DaemonApp::run (int argc, char** argv) {
    ...
    try {
        ...
        if (uninstall && (message.find ("The service has not been started") !=
            ...
            // it?!). HACK: this message happens intermittently, not sure where
            // from but it's quite misleading for the user. they thing something
            // has gone horribly wrong, but it's just the service manager
            // reporting a false positive (the service has actually shut down in
            // most cases).
        } else {
