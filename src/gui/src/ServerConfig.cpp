...
bool
ServerConfig::save (const QString& fileName) const {
    QFile file (fileName);
...
void
ServerConfig::save (QFile& file) const {
...
ServerConfig::loadSettings () {
    ...
    setRelativeMouseMoves (
        settings ().value ("relativeMouseMoves", false).toBool ());
...
int
ServerConfig::adjacentScreenIndex (int idx, int deltaColumn,
                                   int deltaRow) const {
...
int
ServerConfig::autoAddScreen (const QString name) {
    int serverIndex = -1;
    int targetIndex = -1;
...
bool
ServerConfig::findScreenName (const QString& name, int& index) {
    bool found = false;
    for (int i = 0; i < screens ().size (); i++) {
        if (!screens ()[i].isNull () &&
...
bool
ServerConfig::fixNoServer (const QString& name, int& index) {
    bool fixed = false;
...
int
ServerConfig::showAddClientDialog (const QString& clientName) {
