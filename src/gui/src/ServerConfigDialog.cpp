...
ServerConfigDialog::on_m_pListHotkeys_itemSelectionChanged () {
    ...
    if (itemsSelected && serverConfig ().hotkeys ().size () > 0) {
        ...
        // deleted the next to last item, so idx can only possibly be 0. GDB
        // shows we got called indirectly from the delete line in
        // on_m_pButtonRemoveHotkey_clicked() above, but the delete is of course
        // necessary and seems correct. The while() is a generalized workaround
        // for all that and shouldn't be required.
